$(document).ready(function () {
	...
	var wrongAnswerCount;
		...
		for(i=0;i<data.listedemot.length;i++){ 
			questionBank[i]=new Array;
			questionBank[i][0]=data.listedemot[i].mot;
			questionBank[i][1]=data.listedemot[i].indice;
		}
		titleScreen();
		})//gtjson
...
function gameScreen(){
	...
	$('#gameContent').append('<div id="motdetenteur"></div>');
	$('#gameContent').append('<div id="indicedetenteur"></div>');
	$('#gameContent').append('<div id="estimation">Estimations précédentes:</div>');
	...
	for(i=0;i<numberOfTiles;i++){
		$('#motdetenteur').append('<div class="tile" id=t'+i+'></div>');
	}
			
	$('#indicedetenteur').append("ALLUSION: "+currentClue);
...
function defeatMessage(){
	document.activeElement.blur();
	$(document).off("keyup", handleKeyUp);
	$('#feedback').append("Tu es mort!<br>(answer= "+ currentWord +")<div id='replay' class='button'>CONTINUE</div>");
	$('#replay').on("click",function (){
		if(questionBank.length>0){
			gameScreen()}
		else{finalPage()}
...
function finalPage(){
	$('#gameContent').empty();
	$('#gameContent').append('<div id="finalMessage">Vous avez terminé tous les mots du jeu!</div>');
}//finalpage
